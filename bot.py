...
# هوش مصنوعی Gemini (اختیاری، برای دستور /ask)
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
GEMINI_MODEL = os.getenv("GEMINI_MODEL", "gemini-3.5-flash")
gemini_client = None
if GEMINI_AVAILABLE and GEMINI_API_KEY:
    gemini_client = genai.Client(api_key=GEMINI_API_KEY, http_options=HttpOptions(api_version="v1"))

# --- هوش مصنوعی (Gemini) برای دستور /ask ---
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
gemini_client = None
if GEMINI_API_KEY:
...
# ترجیحاً INSTAGRAM_COOKIES_B64 (نسخه Base64، امن‌تر موقع پیست کردن تو پنل‌های وب).
# اگه نبود، از INSTAGRAM_COOKIES (متن خام) استفاده می‌کنه.
...
_cookie_content = ""
if INSTAGRAM_COOKIES_B64.strip():
    try:
        _cookie_content = _b64.b64decode(INSTAGRAM_COOKIES_B64.strip()).decode("utf-8")
    except Exception as _e:
        logging.getLogger("bot").error("Could not decode INSTAGRAM_COOKIES_B64: %s", _e)
elif INSTAGRAM_COOKIES.strip():
    _cookie_content = INSTAGRAM_COOKIES

if _cookie_content.strip():
    with open(INSTAGRAM_COOKIES_PATH, "w", encoding="utf-8") as f:
        f.write(_cookie_content)
else:
...
async def cmd_ask(message: Message):
    ...
    if not gemini_client:
        await message.answer("این قابلیت فعلاً روشن نیست (کلید Gemini تنظیم نشده).")
        return

    question = message.text.replace("/ask", "", 1).strip()
    if not question:
        await message.answer("بعد از /ask سوالتو بنویس، مثلاً:\n/ask پایتخت فرانسه کجاست؟")
        return
    ...
    try:
        ...
        await message.answer("⚠️ خطا تو گرفتن جواب از هوش مصنوعی، دوباره امتحان کن.")
        return

    # تلگرام پیام‌های بالای ۴۰۹۶ کاراکتر رو رد می‌کنه، پس در صورت نیاز تکه‌تکه می‌فرستیم
